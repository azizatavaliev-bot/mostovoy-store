...
const config = {
  ...
  // Значения ниже — стартовый дефолт; server/services/fx-rates.js раз в 6 часов
  // подтягивает реальный курс и обновляет этот же объект на месте. Если внешний
  // источник недоступен, остаются эти цифры (или то, что было получено раньше).
  rates: {
    base: "USD",
    USD: 1,
    KGS: Number(process.env.RATE_USD_KGS) || 83,
