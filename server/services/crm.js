...
const instagramStore = require("./instagram-integration-store");
const wabery = require("./wabery");
const { buildMapping } = require("./privacy");
...
// Модель всегда пишет жирный текст как **markdown** (это то, что превращает
// в <b> telegramHtml выше) — но у WhatsApp своя разметка: жирный там ровно
// одна звёздочка (*текст*), а **текст** он не понимает и показывает две
// звёздочки буквально. Поэтому перед отправкой в WhatsApp просто снимаем
// одну звёздочку с каждой стороны, не трогая единичные звёздочки — их модель
// в жирный текст не превращает, а вот WhatsApp воспримет их как курсив/жирный
// сам по себе, так что тут ничего ломать не нужно.
function whatsappMarkdown(text) {
  // WhatsApp дополнительно требует, чтобы звёздочка стояла ВПЛОТНУЮ к
  // тексту — «* текст*» (с пробелом после звёздочки) жирным не станет,
  // звёздочки просто останутся видны буквально. Модель иногда пишет
  // markdown-список через «** пункт**» с пробелом (обычный стиль markdown,
  // где пробел после маркера не мешает) — здесь пробел внутри убираем,
  // а не только саму звёздочку дублируем/схлопываем.
  return String(text || "").replace(/\*\*\s*([^*\n]+?)\s*\*\*/g, "*$1*");
}

const DEFAULT_PROMPT = `Ты продавец-консультант магазина техники МОСТОВОЙ в Бишкеке.
Адрес магазина: ТЦ ЦУМ «Айчурек», проспект Чуй, 155, 1 этаж, отделы A9 и D14.
...
Помоги выбрать товар и ответь на вопрос. Не предлагай оформить заказ сам — только если клиент сам об этом попросит. Не упоминай, что ты AI.`;
...
Отвечай по существу вопроса живым языком, а не сухим справочником — но не добавляй от себя предложение следующего шага («оформить?», «зарезервировать?», «рассчитать рассрочку?»), если клиент сам об этом не спросил. Дай полезный ответ и на этом остановись.
...
Если клиент просит оформить заказ И В ТОМ ЖЕ сообщении уже назвал модель и конфигурацию (например «можно оформить? 17 256 esim») — не переспрашивай то, что он только что сам сказал. Через search_catalog подтверди именно эту модель с ценой и попроси только то, чего не хватает для оформления: имя, телефон, город и адрес доставки (адрес не нужен, если клиент явно хочет забрать в шоу-руме).
КРИТИЧЕСКИ ВАЖНО про адрес доставки: копируй название улицы СИМВОЛ В СИМВОЛ из сообщения клиента, даже если оно похоже на опечатку в известном тебе названии. Пример: клиент написал «бишкек вавилоа 18» — в подтверждении заказа должно быть написано именно «вавилоа 18», а НЕ «Вавилова 18», даже если тебе кажется, что клиент имел в виду улицу Вавилова. Ты не знаешь точно, что он имел в виду — это может быть другая улица или клиент действительно так её называет. Автозамена на «правильное», по твоему мнению, название — это порча реального адреса доставки в заказе, а не помощь. Единственное исключение — если название совсем нечитаемо (набор случайных символов) или отсутствует: тогда прямо спроси у клиента точное название улицы, не подставляя свою версию.
Перед тем как доставать из сообщения клиента имя/телефон/город/адрес и подтверждать заказ, убедись, что сообщение вообще про оформление. Если клиент прислал что-то не связанное с заказом (случайная просьба вроде «напиши код на питоне», вопрос не по теме, шутка, бессмыслица) — НЕ пытайся выискать в этом тексте имя, телефон или адрес и НЕ пиши «зафиксировал ваш заказ» или подобное. Просто ответь по существу того, что клиент реально написал (например, вежливо скажи, что этим не занимаешься) — даже если до этого в диалоге уже называлась цена товара.`;
const DEFAULT_TASK_PROMPT = `Помоги клиенту выбрать подходящий товар и ответь на вопрос. Не начинай с вопросов, если уже можно показать подходящие варианты. Не предлагай оформить заказ или резерв сам — если клиент захочет оформить, тогда попроси имя и удобный способ связи.`;
...
- не добавляет от себя предложение оформить заказ, зарезервировать или рассчитать рассрочку/Trade-in, если клиент сам об этом не спрашивал;
...
- если в черновике есть адрес, город, имя или телефон клиента — они переписаны точно как в черновике, символ в символ. Даже если название улицы похоже на опечатку в известном тебе слове, НЕ исправляй его на «правильное» — ты не знаешь, что имел в виду клиент, а неверный адрес в заказе это реальная ошибка доставки.
...
// Те же кнопки-ссылки, что в закреплённом посте канала @mostovoyshopp —
// каждая ведёт на конкретный пост об этой категории (t.me/mostovoyshopp/<id>).
// Ссылки статичные (снятые с самого поста): при смене ассортимента их надо
// обновить руками здесь и в закрепе канала, синхронизации с постом нет.
// ВРЕМЕННО (placeholder): для Beats и наушников Sony отдельного поста нет —
// по словам владельца, позже будет один общий пост на AirPods/Beats/Sony,
// пока кнопка ведёт на пост с AirPods (там Beats тоже упомянут).
const CHANNEL_FALLBACK_URL = "https://t.me/mostovoyshopp";
const TELEGRAM_CATEGORY_BUTTON_ROWS = [
  [{ text: "iPhone 18", url: "https://t.me/mostovoyshopp/447" }, { text: "iPhone", url: "https://t.me/mostovoyshopp/444" }],
  [{ text: "SAMSUNG", url: "https://t.me/mostovoyshopp/443" }, { text: "XIAOMI", url: "https://t.me/mostovoyshopp/442" }, { text: "Google Pixel", url: "https://t.me/mostovoyshopp/397" }],
  [{ text: "Apple Watch", url: "https://t.me/mostovoyshopp/438" }, { text: "MacBook", url: "https://t.me/mostovoyshopp/440" }, { text: "IPad", url: "https://t.me/mostovoyshopp/441" }],
  [{ text: "DYSON", url: "https://t.me/mostovoyshopp/435" }],
  [{ text: "GARMIN", url: "https://t.me/mostovoyshopp/437" }],
  [{ text: "DJI & Insta360", url: "https://t.me/mostovoyshopp/452" }],
  [{ text: "Ray Ban, Oakley Meta glasses", url: "https://t.me/mostovoyshopp/432" }],
  [{ text: "AirPods | Beats | Sony", url: "https://t.me/mostovoyshopp/439" }],
  [{ text: "Игровые приставки", url: "https://t.me/mostovoyshopp/431" }],
  [{ text: "Бритвы Philips", url: "https://t.me/mostovoyshopp/399" }],
  [{ text: "Фитнес браслеты (Fitbit, Whoop)", url: "https://t.me/mostovoyshopp/396" }],
  [{ text: "Яндекс Алиса колонка", url: "https://t.me/mostovoyshopp/433" }],
  [{ text: "Canon g7x mark 3", url: "https://t.me/mostovoyshopp/398" }],
  [{ text: "Trade in  телефоны", url: "https://t.me/mostovoyshopp/402" }],
  [{ text: "РАССРОЧКА ZERO", url: "https://t.me/mostovoyshopp/394" }],
  [{ text: "Рассрочка без банка", url: "https://t.me/mostovoyshopp/404" }],
];
const TELEGRAM_CATEGORY_KEYBOARD = { inline_keyboard: TELEGRAM_CATEGORY_BUTTON_ROWS };
...
// а не ждёт молча. По просьбе владельца магазина цепочка мягче старой (не
// давит на каждой ступени продажей): день 1 — два коротких напоминания,
// день 1+3 — ещё одно, и через неделю после него — одно финальное сообщение
// без призыва купить («можете писать в любое время»). После финального шага
// бот сам больше НИЧЕГО не пишет — только отвечает, если клиент вернётся.
const NUDGE_DAY1_DELAY = "+1 day";
const NUDGE_DAY2_DELAY = "+2 days";
const NUDGE_DAY3_DELAY = "+5 days";
const NUDGE_LAST_DELAY = "+12 days";
...
function isLikelyHumanName(value) {
  ...
  // Раньше требовался «https:» с двоеточием — профиль с именем просто
  // «https» (без «://...», обрезанное или так и есть целиком) проходил
  // проверку и подставлялся в шаблон как есть (найдено на проде:
  // «Здравствуйте, https 😊 Хотела уточнить...»). https/http теперь матчится
  // как подстрока, без обязательного двоеточия.
  if (/https?|www\.|t\.me\/|@|\d{3,}/i.test(trimmed)) return false;
  return true;
}

// Свои люди (KNOWN_CONTACTS в конфиге) не должны получать автопродажу —
// список хранит вперемешку номера, telegram-юзернеймы и числовые chat id,
// поэтому сравниваем диалог по всем трём полям сразу, а не гадаем формат
// записи. Номер сравнивается по цифрам без учёта +/пробелов/скобок, юзернейм
// без учёта регистра и «@».
function isKnownContact(conversation) {
  const list = config.knownContacts;
  if (!list?.length) return false;
  const phoneDigits = String(conversation.customer_phone || "").replace(/\D/g, "");
  const chatIdDigits = String(conversation.external_chat_id || "").replace(/\D/g, "");
  const username = String(conversation.customer_username || "").replace(/^@/, "").toLowerCase();
  // Имя профиля (Telegram first_name+last_name / WhatsApp контакт-имя) —
  // клиент мог сам его придумать, это не такой надёжный признак, как номер
  // или юзернейм, но для записи вроде «Рамазан» этого обычно достаточно:
  // сверяем ПО СЛОВАМ (не подстрокой), чтобы «Алия» не поймала «Али».
  const nameWords = String(conversation.customer_name || "").toLowerCase().split(/\s+/).filter(Boolean);
  return list.some((entry) => {
    const entryDigits = entry.replace(/\D/g, "");
    const entryUsername = entry.replace(/^@/, "").toLowerCase();
    const entryLower = entry.toLowerCase();
    if (entryDigits.length >= 6 && (entryDigits === phoneDigits || entryDigits === chatIdDigits)) return true;
    if (entryUsername && username && entryUsername === username) return true;
    if (nameWords.length && nameWords.includes(entryLower)) return true;
    return false;
  });
}
...
const NUDGE_TEMPLATES = {
  day1: "Здравствуйте, {{client_name}} 😊 Хотела уточнить, остались ли у вас вопросы по {{product_name}}? Могу коротко рассказать подробнее или помочь подобрать другой вариант.",
  day2: "Добрый день, {{client_name}}! Вы интересовались {{product_name}}. Подскажите, вы ещё рассматриваете его или пока решили отложить покупку? 😊",
  ...
  day3: "{{client_name}}, не хочу вас беспокоить 🙂 Если вопрос по {{product_name}} ещё актуален — просто напишите, продолжим с того места, где остановились.",
  // Финальное сообщение цепочки: без призыва купить и без имени товара — это
  // не ещё один дожим, а вежливое «на этом я умолкаю». После него бот сам
  // ничего не пишет вообще, пока клиент не напишет первым.
  last: "{{client_name}}, больше не буду писать первой 🙂 Если появятся вопросы по любому товару — просто напишите в любое время, я на связи.",
...
const REACTIVE_TEMPLATES = [
  {
    ...
    text: "Хорошо, понимаю 😊 Если появятся вопросы — пишите в любое время.",
  },
  {
    kind: "expensive",
    pattern: /дорог(?:о|овато|ая|ой)|не\s+потяну|не\s+укладываюсь\s+в\s+бюджет|дешевле\s+есть|скидк[уи]\s+можно/iu,
    text: "Понимаю вас 😊 Если что-то понадобится — пишите, помогу.",
  },
];
...
const MOSTOVOY_SALES_TEMPLATES = {
  location: {
    ru: "Мы находимся в Бишкеке: ТЦ ЦУМ «Айчурек», проспект Чуй, 155, 1 этаж, отделы A9 и D14. Хотите приехать в шоу-рум или оформить доставку?",
    ky: "Биз Бишкекте, ЦУМ «Айчурек» соода борборундабыз: Чүй проспекти, 155, 1-кабат, A9 жана D14 бөлүмдөрү. Шоурумга келесизби же жеткирүү керекпи?",
  },
  delivery: {
...
function classifySalesTemplate(text) {
  ...
  // Если клиент прямо в этом же сообщении уже назвал конфигурацию
  // (цифры — объём памяти, «esim»/«sim», размер часов и т.п. — «а можно
  // оформить? 17 256 esim»), готовый шаблон «пришлите модель с памятью и
  // цветом» звучит так, будто бот не прочитал вопрос: он же только что
  // это назвал. В этом случае лучше обычная генерация с search_catalog —
  // она подтвердит именно эту модель и попросит только то, чего не
  // хватает (имя, телефон, город), а не всё заново.
  // Одно число (например «iPhone 17») — это ещё просто модель, не полная
  // конфигурация. А вот два и больше отдельных числа («17 256») или явное
  // «esim»/«sim» — это уже реальная конфигурация, которую не нужно
  // переспрашивать заново.
  const numberGroups = normalized.match(/\d+/gu) || [];
  const hasConfigInSameMessage = numberGroups.length >= 2 || /\besim\b|\bsim\b/iu.test(normalized);
  if (!hasConfigInSameMessage && /оформ(?:ить|ляйте)|хочу\s+(?:заказать|купить)|можно\s+(?:заказать|оформить)/iu.test(normalized)) kind = "order";
...
// Однозначная цифра («Ultra 3» против «Ultra 2», «AirPods 3» против «AirPods
// 4») — токен длиной 1, раньше фильтр «t.length >= 2» его целиком выбрасывал:
// запрос «Apple Watch Ultra 3» превращался в «apple watch ultra» без всякой
// цифры, Ultra 2 и Ultra 3 получали одинаковый счёт, и при равенстве Ultra 2
// (их в каталоге больше) забивал топ-12 результатов, а Ultra 3 туда не
// попадала вообще (найдено на проде: бот утверждал, что Ultra 3 «пока не
// поступала», хотя она есть в наличии). Для одиночной цифры matches ищется
// по границе (не как подстрока «3» внутри «2024» или «13»), для остальных
// токенов — как раньше, обычным includes.
function tokenMatchesHaystack(haystack, token) {
  if (/^\d+$/.test(token) && token.length <= 2) {
    return new RegExp(`(?<![a-zа-я0-9])${token}(?![a-zа-я0-9])`, "u").test(haystack);
  }
...
function searchCatalogProducts(db, query) {
  ...
  const scored = products.map((p) => {
    ...
    const score = tokens.reduce((sum, token) => sum + (tokenMatchesHaystack(haystack, token) ? 1 : 0), 0);
    return { p, score };
...
// (телефоны, планшеты, ноутбуки) — только там в закрывающей фразе
// _categoryBrowseReply уместно упоминать «объём памяти». У AirPods, часов,
// колонок и прочего памяти как характеристики нет вообще (найдено на
// проде: список AirPods заканчивался фразой «назову... объём памяти»).
const CATALOG_FAMILIES = [
  { request: /айфон|iphone/i, terms: ["iphone"], hasStorage: true },
  { request: /макбук|macbook/i, terms: ["macbook"], hasStorage: true },
  { request: /айпад|ipad/i, terms: ["ipad"], hasStorage: true },
  { request: /airpods|аирпод|эирпод/i, terms: ["airpods"] },
  { request: /apple watch|эпл вотч|часы apple/i, terms: ["apple watch"] },
  { request: /samsung|самсунг/i, terms: ["samsung", "galaxy"], hasStorage: true },
  { request: /xiaomi|сяоми|poco/i, terms: ["xiaomi", "poco"], hasStorage: true },
...
// Реальный адрес шоу-рума раньше был только в тексте одного готового
// шаблона (classifySalesTemplate, kind "location"), который триггерится
// узким regex. Стоит клиенту спросить адрес непривычной формулировкой (или
// на кыргызском не тем словом) — regex не срабатывает, ответ уходит в
// обычную генерацию, а там модель адреса вообще не знает и придумывает
// похожий (найдено на проде: реальный адрес «просп. Чуй 155, ТЦ ЦУМ
// Айчурек» превратился в выдуманный «Чүй проспектиси, 123»). Теперь адрес —
// факт в самом промпте, доступный в любой генерации независимо от того,
// сработал ли шаблон.
const ASSISTANT_STORE_FACTS = `АДРЕС МАГАЗИНА:
Шоу-рум находится по адресу: Бишкек, ТЦ ЦУМ «Айчурек», проспект Чуй, 155, 1 этаж, отделы A9 и D14. Если клиент на любом языке спрашивает адрес, как найти магазин или где забрать заказ — называй именно этот адрес. Никогда не придумывай другой адрес или номер дома.`;
...
// Маркер разбивки ответа на несколько сообщений подряд — так же, как живой
// продавец переписывается: не одной длинной «портянкой», а короткими
// репликами. Разбирается в _deliverReply, между частями — пауза и «печатает».
const NEXT_MESSAGE_MARKER = "[NEXT_MESSAGE]";
const ASSISTANT_MULTI_MESSAGE_POLICY = `РАЗБИВКА НА НЕСКОЛЬКО СООБЩЕНИЙ:
Если ответ по смыслу состоит из двух естественных частей (например: сначала цена и факт по товару, отдельно — уточняющий вопрос; или сначала реакция на сообщение клиента, отдельно — сама информация) — можешь разбить его на 2, максимум 3 коротких сообщения вместо одного длинного. Вставь между частями маркер ${NEXT_MESSAGE_MARKER} на отдельной строке — система сама отправит их друг за другом с небольшой паузой, как будто печатает живой человек.
Используй это не в каждом ответе, а только когда несколько коротких сообщений звучат естественнее одного. Не разбивай одно предложение пополам и не превращай список товаров в отдельные сообщения по одному пункту.`;

// По прямой просьбе владельца магазина дожима больше нет вообще: бот только
// отвечает на вопросы клиента и не предлагает следующих шагов сам — ни
// «оформить заказ?», ни резерв, ни рассрочку, ни Trade-in, ни работу с
// возражениями («дорого», «подумаю») в духе продажника. Раньше здесь было
// два разных набора инструкций (обычный и «мягкий» по сегменту лида) —
// оставлен один и тот же нейтральный вариант для всех.
const ASSISTANT_NO_PUSH_POLICY = `БЕЗ ДОЖИМА:
Отвечай только на то, что реально спросил клиент. Не предлагай сам оформить заказ, резерв, рассрочку или Trade-in, не спрашивай «определились с выбором?» и не подталкивай к следующему шагу — только если клиент сам об этом попросил или прямо спросил, что делать дальше.
Если клиент пишет «дорого», «подумаю» или «позже» — просто ответь по существу (например, назови более доступный вариант, если он есть) и оставь решение клиенту, без давления и без повторных предложений.
Запрещено: создавать срочность; повторять предложение оформить заказ; писать, что товар скоро закончится, если этого нет в данных.`;

// По просьбе владельца магазина: без дожима — не значит без личного мнения.
// Раньше на «покажите ноутбуки»/«что посоветуете» бот выдавал сухой список
// через тире с ценами и на этом замолкал. Эта политика требует настоящей
// консультации — выбрать фаворита и сказать об этом от своего имени, живо
// продолжать разговор — но явно не переопределяет ASSISTANT_NO_PUSH_POLICY:
// рекомендация это мнение о товаре, а не «оформляем?».
const ASSISTANT_ADVISORY_POLICY = `КАК СОВЕТОВАТЬ:
Ты живой консультант, а не прайс-лист. Когда показываешь клиенту два и более вариантов — не ограничивайся перечислением через тире с ценами. Выбери из них тот, который сам считаешь наиболее удачным для этого клиента, и прямо скажи об этом своими словами, например: «Я бы лично взял вот этот — ...», «Из этих вариантов самый удачный, на мой взгляд, — ...», «Обычно в этом бюджете чаще всего берут ...», и коротко объясни почему (баланс цены и возможностей, самая ходовая модель, лучше подходит под то, что назвал клиент). Остальные варианты тоже покажи с ценами, но без такого же акцента. Не повторяй одну и ту же формулировку рекомендации в соседних сообщениях — меняй её.
Это не «дожим»: рекомендация — твоё мнение о товаре, а не предложение оформить заказ, поторопиться или «зарезервировать, пока есть». Правила БЕЗ ДОЖИМА выше остаются в силе без исключений.
Веди себя как живой собеседник, а не как автоответчик, который закрывает вопрос и замолкает: после ответа по существу можно естественно продолжить разговор — уточнить, для чего нужен товар, кому он, что важнее (цена, автономность, камера, размер), сравнить варианты между собой по этим критериям или спросить мнение клиента о предложенном варианте. Это обычный интерес консультанта к задаче клиента, а не «определились с выбором?».`;
...
function toConversation(row) {
  return {
    ...
    dealStage: row.deal_stage || "browsing",
    dealStageProduct: row.deal_stage_product || null,
  };
}


// Стадии сделки идут строго по возрастанию: browsing (ещё смотрит) →
// advising (бот уже спросил бюджет/потребность) → configured (search_catalog
// подтвердил конкретный товар с ценой) → checkout (собираем имя/телефон/
// адрес) → handed_off (заказ передан менеджеру). Раньше готово ли спросить
// имя и телефон решалось заново каждым сообщением по regex/памяти модели —
// теперь это стадия в БД, которая продвигается только конкретным событием.
const DEAL_STAGE_ORDER = ["browsing", "advising", "configured", "checkout", "handed_off"];

function dealStageAtLeast(stage, target) {
  return DEAL_STAGE_ORDER.indexOf(stage || "browsing") >= DEAL_STAGE_ORDER.indexOf(target);
}
...
class CrmService {
  ...
  // По прямой просьбе владельца магазина бот больше не пишет клиенту первым
  // вообще — только отвечает на сообщения. Раньше здесь ставилась цепочка
  // напоминаний о простое (день 1/2/3/финал); сама инфраструктура
  // (processDueNudgeFollowUps, _cancelNudgeFollowUps, таблица
  // nudge_follow_ups) оставлена нетронутой на случай, если понадобится
  // включить обратно, — просто ничего новое сюда больше не кладём.
  _scheduleNudgeFollowUps(_conversationId, _productName) {}

  // Тем же образом отключено разовое напоминание «клиент сказал «беру», но
  // не дал контакты» — см. комментарий у _scheduleNudgeFollowUps выше.
  _scheduleOrderIncompleteNudge(_conversationId, _productName) {}
  ...
  // Отключено вместе с остальными проактивными напоминаниями (см.
  // _scheduleNudgeFollowUps) — только снимаем то, что уже могло быть
  // поставлено раньше, ничего нового не ставим.
  ...
  _publishOrderIfConfirmed(conversation, history, selection) {
    ...
    // Раньше «менеджер свяжется с вами» никуда не сохранялось внутри самого
    // бота — только уходило во внешнюю CRM, если она вообще настроена
    // (CRM_BASE_URL). Проверяем стадию ДО _advanceDealStage: если сделка уже
    // была handed_off, это повторное срабатывание того же подтверждения
    // (клиент мог написать «да» ещё раз) — новую строку в crm_orders не
    // создаём, а вот остальное (забота о клиенте, внешняя CRM) не меняем.
    const wasAlreadyHandedOff = this.db.prepare("SELECT deal_stage FROM crm_conversations WHERE id = ?").get(conversation.id)?.deal_stage === "handed_off";
    this._advanceDealStage(conversation.id, "handed_off");
    ...
    if (!wasAlreadyHandedOff) {
      // Адрес — тем же способом, что и маскировка PII перед отправкой в ИИ
      // (services/privacy.js): улица копируется как её реально написал
      // клиент, без попытки «исправить» до похожего известного названия.
      const addressMapping = buildMapping([customerText]);
      const addressEntry = [...addressMapping.entries()].find(([, placeholder]) => placeholder.startsWith("{{ADDRESS"));
      this.db.prepare(
        `INSERT INTO crm_orders (conversation_id, source, product_name, amount, currency, order_type, customer_name, customer_phone, customer_address)
         VALUES (?, ?, ?, ?, 'KGS', ?, ?, ?, ?)`
      ).run(
        conversation.id,
        conversation.source,
        [product.name, product.storage, product.color].filter(Boolean).join(", "),
        amount,
        orderType,
        conversation.customer_name || conversation.customerName || null,
        phone,
  ...
  // Двигает стадию сделки только вперёд (никогда не откатывает назад
  // случайным сообщением) и запоминает подтверждённый товар — имя+память+
  // цвет+цена, ровно то, что реально пришло от search_catalog, а не то, что
  // бот решил написать в тексте.
  _advanceDealStage(conversationId, stage, product = null) {
    if (!conversationId) return;
    const row = this.db.prepare("SELECT deal_stage FROM crm_conversations WHERE id = ?").get(conversationId);
    if (!row) return;
    const current = row.deal_stage || "browsing";
    if (DEAL_STAGE_ORDER.indexOf(stage) < DEAL_STAGE_ORDER.indexOf(current)) return;
    const productJson = product ? JSON.stringify(product) : null;
    this.db.prepare(
      "UPDATE crm_conversations SET deal_stage = ?, deal_stage_product = COALESCE(?, deal_stage_product), updated_at = datetime('now') WHERE id = ?"
    ).run(stage, productJson, conversationId);
  }
  ...
  // Страница «Заказы» в админке: всё, что бот подтвердил в переписке,
  // независимо от того, настроена ли внешняя CRM (см. _publishOrderIfConfirmed).
  listOrders({ status } = {}) {
    const rows = this.db.prepare(
      `SELECT o.*, c.external_key, c.external_chat_id, c.source AS conversation_source
         FROM crm_orders o
         LEFT JOIN crm_conversations c ON c.id = o.conversation_id
  ...
  updateOrder(id, patch) {
    const row = this.db.prepare("SELECT id FROM crm_orders WHERE id = ?").get(Number(id));
    if (!row) return null;
    const status = ["new", "contacted", "confirmed", "cancelled"].includes(patch?.status) ? patch.status : undefined;
    const note = patch?.note === undefined ? undefined : String(patch.note || "").slice(0, 2000);
    if (status === undefined && note === undefined) return this.listOrders().find((o) => o.id === Number(id)) || null;
    this.db.prepare(
      `UPDATE crm_orders SET
       ...
       WHERE id = ?`
    ).run(status ?? null, note ?? null, Number(id));
    return this.listOrders().find((o) => o.id === Number(id)) || null;
  }
  ...
  async approveReply(id, text) {
    ...
    await this._deliverReply(Number(row.conversation_id), finalText, "assistant");
    this.db.prepare(
...
Если замечание относится лишь к единичному случаю, верни пустой prompt_patch.`,
  ...
  async _chatWithCatalogTool({ system, messages, user, model, onUsage, conversationId = null }) {
    ...
    try {
      ...
      if (lastExactProduct && conversationId) {
        this._advanceDealStage(conversationId, "configured", {
          name: lastExactProduct.name,
          price: lastExactProduct.price,
          currency: lastExactProduct.currency,
        });
      }
  ...
  _composePrompt(settings, catalog) {
    return [
      ...
      `ЗАДАЧА:\n${settings.taskPrompt}`,
      ASSISTANT_STORE_FACTS,
      ASSISTANT_PRICE_POLICY,
      ASSISTANT_COLOR_POLICY,
      ASSISTANT_TONE_POLICY,
      ASSISTANT_MULTI_MESSAGE_POLICY,
      ASSISTANT_NO_PUSH_POLICY,
  ...
  async receiveTelegram(message) {
    ...
    if (inserted && isKnownContact(conversation)) {
      this._logEvent(conversation.id, "info", "inbox", "message.known_contact_skipped", "Свой контакт — автоответ не отправляется, только запись в CRM");
    } else if (inserted && conversation.ai_enabled) {
      if (this._isDuplicateInbound(conversation.id, inserted, text)) {
        ...
        try {
          await this._send(conversation.id, FIRST_CONTACT_WELCOME_TEXT, "assistant", { replyMarkup: TELEGRAM_CATEGORY_KEYBOARD });
  ...
  async _handleInboundText(conversation, { externalMessageId, text, raw, createdAt, sourceLabel, isNewConversation, immediate = false, bypassApproval = false }) {
    ...
    if (stageAction) this._publishStage(conversation, stageAction);
    if (classifyImportantEscalation(text)) this._publishImportantNotify(conversation, text);
    if (isKnownContact(conversation)) {
      this._logEvent(conversation.id, "info", "inbox", "message.known_contact_skipped", "Свой контакт — автоответ не отправляется, только запись в CRM");
  ...
  // ── Instagram Direct (официальный Meta Graph API) ─────────────────────
  // Отдельно от receiveAmo/source="instagram" выше — тот канал приходит
  // через amoCRM, этот подключается напрямую через OAuth (см.
  // server/services/instagram-graph.js). Один магазин — одно подключение.
  getInstagramStatus() {
    return { ...instagramStore.getPublicStatus(this.db), configured: config.features.instagramDirect };
  }

  connectInstagram() {
    const state = instagramGraph.createOAuthState(this.db);
    return { url: instagramGraph.buildAuthorizeUrl(state) };
  }

  async handleInstagramCallback({ code, state, error, errorDescription }) {
    if (error) {
      throw new instagramGraph.InstagramApiError("INSTAGRAM_AUTH_FAILED", errorDescription || error);
    }
    if (!instagramGraph.consumeOAuthState(this.db, state)) {
      throw new instagramGraph.InstagramApiError("INSTAGRAM_AUTH_FAILED", "Неверный или просроченный state — возможна попытка подделки запроса (CSRF), либо ссылка уже была использована");
    }
    if (!code) {
      throw new instagramGraph.InstagramApiError("INSTAGRAM_AUTH_FAILED", "Meta не передала authorization code");
    }
    const short = await instagramGraph.exchangeCodeForShortLivedToken(code, this.fetchImpl);
    const long = await instagramGraph.exchangeForLongLivedToken(short.accessToken, this.fetchImpl);
    const account = await instagramGraph.fetchAccountInfo(long.accessToken, short.userId, this.fetchImpl).catch(() => ({ username: null }));
    const expiresAt = new Date(Date.now() + long.expiresInSeconds * 1000).toISOString();
    instagramStore.upsertIntegration(this.db, {
      instagramAccountId: short.userId,
      username: account.username,
      accessToken: long.accessToken,
      expiresAt,
      scopes: instagramGraph.OAUTH_SCOPES,
    });
    this._logEvent(null, "info", "instagram", "instagram.connected", "Instagram Direct подключён", { instagramAccountId: short.userId, username: account.username });
    return this.getInstagramStatus();
  }

  disconnectInstagram() {
    instagramStore.deleteIntegration(this.db);
    this._logEvent(null, "warn", "instagram", "instagram.disconnected", "Instagram Direct отключён");
    return { connected: false };
  }

  // Долгоживущий токен живёт 60 дней и может быть продлён (тем же
  // ig_refresh_token механизмом, что и обмен short→long — Meta не различает
  // «обменять» и «продлить», это один и тот же вызов graph.instagram.com).
  // Если продлить не удалось (токен отозван/просрочен) — статус переводится
  // в reauth_required, и админка должна показать кнопку «Переподключить»,
  // а не молча пытаться слать сообщения токеном, который уже не работает.
  async _getValidInstagramAccessToken() {
    const row = instagramStore.getIntegration(this.db);
    if (!row || row.status !== "connected") return null;
    const expiresAt = row.token_expires_at ? new Date(row.token_expires_at) : null;
    const refreshBy = expiresAt ? new Date(expiresAt.getTime() - instagramGraph.TOKEN_REFRESH_MARGIN_DAYS * 86400000) : null;
    if (!refreshBy || Date.now() < refreshBy.getTime()) {
      return instagramStore.getDecryptedAccessToken(this.db);
    }
    try {
      const current = instagramStore.getDecryptedAccessToken(this.db);
      const refreshed = await instagramGraph.exchangeForLongLivedToken(current, this.fetchImpl);
      const newExpiresAt = new Date(Date.now() + refreshed.expiresInSeconds * 1000).toISOString();
      instagramStore.updateAccessToken(this.db, { accessToken: refreshed.accessToken, expiresAt: newExpiresAt });
      this._logEvent(null, "info", "instagram", "instagram.token_refreshed", "Токен Instagram Direct продлён");
      return refreshed.accessToken;
    } catch (error) {
      instagramStore.markReauthRequired(this.db, error.message);
      this._logEvent(null, "warn", "instagram", "instagram.token_refresh_failed", error.message);
      return null;
    }
  }

  // Событие webhook уже нормализовано роутом (server/routes/instagram-webhook.js)
  // до { senderId, text, messageId }. Дедуп по messageId делает сам роут
  // (таблица instagram_webhook_events) — сюда попадают только новые события.
  // Дальше — тот же общий хвост приёма текста, что у WhatsApp/лаборатории
  // (_handleInboundText): дедуп по времени, приветствие новому клиенту,
  // эскалация, debounce перед автоответом — ничего не дублируем заново.
  async receiveInstagramDirect({ senderId, text, messageId }) {
    if (!senderId || !text) return { ignored: true };
    const externalKey = `instagram_direct:${senderId}`;
    const isNewConversation = !this.db.prepare("SELECT 1 FROM crm_conversations WHERE external_key = ?").get(externalKey);
    const conversation = this._upsertConversation({
      externalKey,
  ...
  // приходит уже нормализованное сообщение с их conversation_id — он же
  // становится нашим external_chat_id, ответ шлём тем же id обратно.
  async receiveWabery({ conversationId, text, messageId, username }) {
    if (!conversationId || !text) return { ignored: true };
    const externalKey = `wabery_instagram:${conversationId}`;
    const isNewConversation = !this.db.prepare("SELECT 1 FROM crm_conversations WHERE external_key = ?").get(externalKey);
    const conversation = this._upsertConversation({
      externalKey,
      source: "wabery_instagram",
      inbound: true,
  ...
  _categoryBrowseReply(customerText) {
    ...
    // p.available = 0 — товар был в постах канала, но сейчас не в наличии
    // (см. коммент у getDedupedCatalogProducts). Раньше этот фильтр тут
    // отсутствовал: список линейки показывал вообще все модели, которые
    // когда-либо постились в канал, включая давно распроданные — клиент
    // видел «в наличии несколько моделей» с половиной моделей, которых
    // реально нет (найдено на проде на примере iPad).
    const rows = this.db.prepare(
      `SELECT DISTINCT p.official_name AS name, p.brand AS brand, p.category AS category, p.color AS color, p.specifications AS specifications
         ...
         JOIN message_products mp ON mp.product_id = p.id
         JOIN telegram_messages tm ON tm.id = mp.message_id
        WHERE p.status != 'hidden' AND p.available = 1 AND mp.active = 1 AND tm.is_deleted = 0 AND mp.price IS NOT NULL`
    ).all();
    ...
    const closing = family.hasStorage
      ? "Какая интересует? Назовите модель — сразу назову актуальную цену и объём памяти."
      : "Какая интересует? Назовите модель — сразу назову актуальную цену.";
  ...
  async _autoReply(conversationId, incomingMessageId, options = {}) {
    ...
    let salesTemplate = classifySalesTemplate(latestCustomerMessage);
    // Код-уровневый барьер: готовый шаблон «пришлите имя, телефон...» можно
    // отправлять только если сделка уже дошла до стадии configured — то есть
    // search_catalog где-то в этом диалоге уже подтвердил конкретный товар с
    // ценой. Если клиент пишет «хочу оформить» раньше, чем бот вообще
    // показал цену, шаблон отбрасывается, и сообщение уходит на обычную
    // генерацию ниже, которая сперва покажет товар. Это лечит тот же баг
    // («сразу оформление» на голое «беру»), но на уровне кода, а не только
    // промпта — регресс в этом месте больше не зависит от того, помнит ли
    // модель инструкцию.
    if (salesTemplate?.kind === "order" && !dealStageAtLeast(detail.conversation.dealStage, "configured")) {
      salesTemplate = null;
    }
    if (salesTemplate) {
      ...
      if (salesTemplate.kind === "order") {
        this._advanceDealStage(conversationId, "checkout");
        this._scheduleOrderIncompleteNudge(conversationId, null);
      }
    ...
    try {
      ...
      if (settings.approvalEnabled && !options.bypassApproval) {
        ...
        await this._deliverReply(conversationId, reply, "assistant");
        ...
        if (!orderCareStarted) {
          // Раньше был запасной вариант — selectedCatalogProduct(selection),
          // старая эвристика по узкой подборке каталога: она берёт первый
          // ДОСТУПНЫЙ товар из этой подборки, а не то, что клиент реально
          // обсуждал. Убрали совсем (не только для случая с 2+ найденными
          // товарами) — найдено на проде: клиент спросил просто адрес
          // магазина (ни слова про товар), а напоминание всё равно назвало
          // случайный «PlayStation 5 Slim», потому что он просто был первым
          // доступным в широкой подборке этого диалога. Если search_catalog
          // не сузился до ровно одного реально обсуждаемого товара — значит
          // товар в этом сообщении не обсуждался вообще, и лучше не называть
          // никакой товар (шаблон подставит нейтральное «интересующим вас
          // товаром»), чем угадывать случайный из каталога.
          const product = generated.groundedProducts?.length === 1 ? generated.groundedProducts[0] : null;
  ...
  // Модель может разбить ответ на несколько реплик маркером NEXT_MESSAGE_MARKER
  // (см. ASSISTANT_MULTI_MESSAGE_POLICY) — шлём их по одной с паузой и
  // «печатает…» между ними, как живой человек. Если маркера нет, ведёт себя
  // ровно как раньше: один _send с текстом целиком, без задержки.
  async _deliverReply(conversationId, fullText, sender) {
    const parts = String(fullText || "").split(NEXT_MESSAGE_MARKER).map((part) => part.trim()).filter(Boolean);
    if (parts.length <= 1) {
      await this._send(conversationId, String(fullText || "").trim(), sender);
      return;
    }
    for (let i = 0; i < parts.length; i += 1) {
      if (i > 0) {
        await this._sendTypingAction(conversationId).catch(() => {});
        await this._sleep(600 + Math.floor(Math.random() * 700));
      }
      await this._send(conversationId, parts[i], sender);
    }
  }
  ...
  // Лучшая попытка: индикатор «печатает…» есть только в Telegram Bot API,
  // и даже там сбой не должен ронять доставку самого сообщения.
  async _sendTypingAction(conversationId) {
    const c = this.db.prepare("SELECT * FROM crm_conversations WHERE id = ?").get(conversationId);
    if (!c || c.source !== "telegram" || isLabConversation(c) || !config.telegram.botToken) return;
    await this.fetchImpl(`${config.telegram.apiBase}/bot${config.telegram.botToken}/sendChatAction`, {
      method: "POST",
      headers: { "content-type": "application/json" },
      body: JSON.stringify({ chat_id: c.external_chat_id, action: "typing" }),
    });
  }

  async _send(conversationId, text, sender, { replyMarkup } = {}) {
    const c = this.db.prepare("SELECT * FROM crm_conversations WHERE id = ?").get(conversationId);
    ...
    if (isLabConversation(c)) {
      ...
      if (!this.greenapi?.enabled) throw new Error("Green API не настроен");
      await this.greenapi.sendMessage(toGreenApiChatId(c.external_chat_id), whatsappMarkdown(text));
    ...
    } else if (c.source === "wabery_instagram") {
      await wabery.sendTextMessage({ conversationId: c.external_chat_id, text }, this.fetchImpl);
    } else if (c.source === "instagram_direct") {
      const integration = instagramStore.getIntegration(this.db);
      if (!integration) throw new Error("Instagram Direct не подключён");
      const accessToken = await this._getValidInstagramAccessToken();
      if (!accessToken) throw new Error("Instagram Direct требует переподключения (токен недействителен)");
      try {
        await instagramGraph.sendDirectMessage({
          accessToken,
          igUserId: integration.instagram_account_id,
          recipientId: c.external_chat_id,
          text,
        }, this.fetchImpl);
      } catch (error) {
        if (error.code === "INSTAGRAM_TOKEN_EXPIRED" || error.code === "INSTAGRAM_TOKEN_REVOKED") {
          instagramStore.markReauthRequired(this.db, error.message);
        }
        throw error;
      }
