...
test("шаблон «оформить» уступает обычной генерации, если клиент уже назвал конфигурацию в том же сообщении", () => {
  // Найдено на проде: «а можно оформить? 17 256 esim» получал готовый
  // шаблон «пришлите модель с памятью и цветом», хотя клиент только что
  // это назвал — бот выглядел так, будто не прочитал сообщение.
  assert.equal(classifySalesTemplate("А можно оформить? 17 256 esim"), null, "с конфигурацией — обычная генерация, не готовый шаблон");
  // Одно число — это просто модель, а не полная конфигурация; готовый
  // шаблон по-прежнему уместен.
  assert.equal(classifySalesTemplate("Можно заказать iPhone 17?").kind, "order");
});
